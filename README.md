Data Structures with C 🚀
Welcome to my Data Structures and Algorithms (DSA) repository. Built during my BCA studies, this project is a collection of core programming concepts implemented from scratch in C.

Mastering these foundational concepts—especially memory management, bounds checking, and pointer logic—is a critical stepping stone toward my ultimate career focus in Cybersecurity and Networking. Understanding how data is stored and manipulated at a low level is essential for writing secure, vulnerability-free code.

📌 Project Overview
This repository serves as my personal laboratory where I implement, test, and document various data structures and algorithms.

📁 Currently Implemented:
1. Searching Algorithms

linear_search.C - Basic Sequential Search through an array.

binary_search.C - Optimized Search for sorted arrays using divide-and-conquer.

2. Sorting Algorithms

bubble_sort.c - Standard Bubble Sort implementation.

shall_sort.C - Shell Sort algorithm (utilizing gapped insertion sort for efficiency).

3. Linked Lists

singly_linkedlist.C - Singly Linked List supporting Insertion and Deletion.

dubly_linked_list.C - Doubly Linked List with bidirectional pointer management (prev/next).

4. Linear Data Structures

stack.C - LIFO implementation featuring strict overflow/underflow protection.

deque.C - Double Ended Queue supporting insertion and deletion from both Front and Rear ends.

⏳ Coming Soon:
Standard Queues and Circular Queues

Tree and Graph structures

🛠️ Tech Stack & Tools
Language: C

Compiler: GCC / Turbo C

Environment: VS Code / Windows

🚀 How to Run the Code
Clone the repo:

Bash
git clone https://github.com/devid5111/data_structure_with_c.git
Navigate to the directory:

Bash
cd data_structure_with_c
Compile and run (Example: Deque):

Bash
gcc deque.C -o deque
./deque
(Note: Pre-compiled .exe files are also available in the repository for Windows users).
